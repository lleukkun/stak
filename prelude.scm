...
(define-library (stak vector)
  ...
  (begin
    ...
    (define $bytevector-copy! (primitive 601))
      ...
      (let ((index 0)
        ...
        (define (read-byte)
          (if (< index length)
            ...
            #f))
        (define (read-bulk destination start end)
          (let ((count (min (- end start) (- length index))))
            (bytevector-copy! destination start xs index (+ index count))
    ...
    (define (open-output-bytevector)
      (let ((chunks '())
        ...
        (define (write-bulk xs start end)
          (let loop ((at start))
            (unless (>= at end)
              (let ((count (min (- end at)
                                (- bytevector-output-chunk-size index))))
                (bytevector-copy! chunk index xs at (+ at count))
                (set! index (+ index count))
